...
export class ProListService {
  ...
  getdata(Vary): Observable<ProjectSchema[]> {
    ...
    return this.http.get('http://localhost:8080/' + Vary).map(
      (response: Response) => {
        return <ProjectSchema[]>response.json();
      }
    )
  }
  ...
  getpro(proName): Observable<ProjectSchema[]> {
    ...
    return this.http.get('http://localhost:8080/project/' + proName).map(
      (response: Response) => {
        return <ProjectSchema[]>response.json();
      }
    )
  ...
  postCompany(NewCompany: CompanySchema): Promise<CompanySchema> {
    ...
    //   })
    // };
    return this.http.post('http://localhost:8080/company', NewCompany, options)
      .toPromise()
